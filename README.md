...
Click a size preset to resize the main preview to that exact CSS size; **Fit** restores the large preview. At 16 px and below the face disappears; above 16–24 px shows eyes only; above 24 px shows the full face. Above 16–48 px, facial features render 30% larger with black round eyes and no iris; 24 px remains eyes-only. Larger sizes restore the authored face style. Below 48 px, toon shading and ground shadows are suppressed so the chosen fill or gradient stays clean; 48 px and above retain the saved lighting settings. At 24 px and below, output is always front-facing and position-locked with no body deformation or cursor turning. At 128 px and below, tighter fixed framing fills about 91% of the canvas height for a front-facing body; framing is shared by previews and exports. Display density does not change these thresholds. Marketing export dimensions are configured independently.
