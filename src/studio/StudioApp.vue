...
const poseSource = computed(() => project.value.expressions.find(e => e.id === expression.value.poseExpressionId))
function previewPose(e: Expression) { const source = project.value.expressions.find(s => s.id === e.poseExpressionId) ?? e; return source.beats[Math.min(1, source.beats.length - 1)]!.pose }
function setPoseSource(event: Event) { const id = (event.target as HTMLSelectElement).value; if (id) expression.value.poseExpressionId = id; else delete expression.value.poseExpressionId; frozenBeat.value = false; playing.value = false }
...
function copySection(section: BeatSection) {
  sectionClipboard.value[section] = copyBeatValues(beat.value, section)
  notify(`${section} values copied. Choose another beat and paste from its section menu.`)
}
function pasteSection(section: BeatSection) {
  if (!sectionClipboard.value[section]) return
  flush()
  if (pasteBeatValues(beat.value, section, sectionClipboard.value[section])) {
    frozenBeat.value = true; playing.value = false; flush(); notify(`${section} values pasted. Undo is available.`)
  }
}
...
const previewSizes = [16, 20, 24, 32, 40, 48, 64, 96, 128, 256, 512] as const
...
const loadingPreset = defaultExpressions().find(e => e.id === 'loading')!
const hasLoadingAnimation = computed(() => project.value.animations.some(a => a.steps.length === 1 && project.value.expressions.some(e => e.id === a.steps[0]!.expressionId && isGradientExpression(e))))
function addLoading() {
  flush()
  try {
    const added = addLoadingAnimation(project.value); selectAnimation(added); playing.value = !window.matchMedia('(prefers-reduced-motion: reduce)').matches; flush()
    notify('Loading added. Edit its Rotate and Hold beats in Expressions.')
  } catch (error) { notify(error instanceof Error ? error.message : 'Loading could not be added.') }
}
...
function rotatePreview(value: typeof rotation.value) { beforeFollowing.delete(character.value.id); character.value.trueFront = false; character.value.followRotation = false; rotation.value = value }
...
const sample = computed(() => {
  ...
  if (frozenBeat.value && !playing.value && mode.value === 'expression') return { ...value, pose: poseSource.value ? value.pose : beat.value.pose, faceLayers: poseSource.value ? value.faceLayers : undefined, beatIndex: selectedBeat.value, blink: 0, bob: 0, breathe: 0, effectPhase: undefined, propAmount: 1, tearAmount: 1 }
  return value
})
...
function resetFront() {
  beforeFollowing.delete(character.value.id)
...
async function editExpression(value: Expression, index = 0) { selectExpression(value); activeFaceSet.value = value.beats[index]!.pose.faceSet; editing.value = true; tab.value = 'expressions'; pickBeat(index); await nextTick(); inspectorScroll.value?.scrollTo({ top: 0 }); beatName.value?.focus() }
const expressionName = ref<HTMLInputElement>(), beatName = ref<HTMLInputElement>()
async function renameExpression(value: Expression) { await editExpression(value); expressionName.value?.focus(); expressionName.value?.select() }
function deleteExpression(value: Expression) { flush(); if (!removeExpression(project.value, value.id)) return; normalizeSelections(); playing.value = false; flush(); notify(`${value.name} deleted. Animation references updated; Undo is available.`) }
...
const beforeFollowing = new Map<string, { rotation: typeof rotation.value; trueFront: boolean }>()
function toggleRotationFollowing() {
  const c = character.value
  if (!c.followRotation) {
    beforeFollowing.set(c.id, { rotation: { ...rotation.value }, trueFront: c.trueFront })
    rotation.value = { x: 0, y: 0, z: 0 }; c.trueFront = false; c.followRotation = true
  } else {
    c.followRotation = false
    const previous = beforeFollowing.get(c.id)
    if (previous) { rotation.value = { ...previous.rotation }; c.trueFront = previous.trueFront }
    beforeFollowing.delete(c.id)
  }
}
...
async function importProject(event: Event) {
  ...
  try {
    ...
    const imported = parseProject(JSON.parse(await file.text())); imported.defaultsRevision = 2; flush(); project.value = imported
...
const svgAction = ref<'download' | 'copy' | null>(null)
let controller: AbortController | undefined
const exportDuration = computed(() => animationDuration(animation.value) / character.value.speed * repetitions.value)
watch(exportSize, value => { if (value === 'square') { width.value = 1080; height.value = 1080 } else if (value === 'portrait') { width.value = 1080; height.value = 1350 } else if (value === 'story') { width.value = 1080; height.value = 1920 } else if (value === 'landscape') { width.value = 1920; height.value = 1080 } else if (value === '16') { width.value = 16; height.value = 16 } else if (value === 'icon') { width.value = 128; height.value = 128 } })
...
async function exportSvg(action: 'download' | 'copy') {
  if (busy.value) return
  busy.value = true; svgAction.value = action
  try {
    const name = character.value.name
    const svg = renderSvg(clone(definition.value), { width: width.value, height: height.value, animationId: animation.value.id, background: transparent.value ? null : exportBackground.value, rotation: { ...rotation.value }, zoom: zoom.value, cursor: clone(previewCursor.value), sample: clone(sample.value) })
    if (action === 'copy') {
      try { await navigator.clipboard.writeText(svg); notify('SVG snapshot copied to clipboard.') }
      catch { notify('Clipboard access is unavailable. Use Download SVG instead.') }
    } else { saveBlob(new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }), `${fileName(name)}.svg`); notify('SVG snapshot downloaded.') }
  } catch (error) { notify(error instanceof Error ? error.message : 'The SVG could not be exported.') }
  finally { busy.value = false; svgAction.value = null }
}
...
onMounted(() => { persist(); frameId = requestAnimationFrame(tick); window.addEventListener('keydown', keyboard); window.addEventListener('pagehide', flush) })
...
<template>
  <div class="studio-app">
    ...
    <main class="studio-workspace">
      <section class="workbench" aria-label="Character preview and sequencing">
        ...
        <div class="preview-frame" :class="{ light: previewBackground === 'light', checker: previewBackground === 'transparent' }">
          <Stage :character="character" :sample="sample" :rotation="rotation" :zoom="zoom" :background="background" :playing="playing" :preview-size="previewSize" @rotate="rotatePreview" @reset="resetRotation" @pause="playing = false" @cursor="previewCursor = $event"><template #controls>            <div class="orbit-actions" role="group" aria-label="Preview behavior">
              <button class="orbit-action" :disabled="previewSize !== null && previewSize <= 32" :class="{ active: character.trueFront || (previewSize !== null && previewSize <= 32) }" aria-label="Front reset" title="Keep the character facing straight ahead, including during animation" @click="resetFront"><Icon name="front" :size="16" /><span>Front reset</span></button>
              <button class="orbit-action" :disabled="previewSize !== null && previewSize <= 32" :class="{ active: character.lockPosition || (previewSize !== null && previewSize <= 32) }" aria-label="Lock position" :aria-pressed="character.lockPosition" title="Stop floating and breathing; keep expressions and details moving" @click="character.lockPosition = !character.lockPosition"><Icon :name="character.lockPosition ? 'lock' : 'unlock'" :size="16" /><span>Position</span></button>
        ...
        <section class="timeline" aria-label="Animation sequence">
          ...
          <div class="sequence-track" v-else>
            <div v-for="(item, index) in animation.steps" :key="item.id" :class="['sequence-beat', { active: sample.stepIndex === index }]" draggable="true" @dragstart="dragIndex = index" @dragover.prevent @drop.prevent="dropAt(index)">
              <button class="beat-main" @click="time = animation.steps.slice(0, index).reduce((n, s) => n + s.duration, 0) / character.speed; playing = false; frozenBeat = false"><Thumb :character="character" :pose="previewPose(stepExpression(item.expressionId))" :size="52" /><span><strong>{{ stepExpression(item.expressionId).name }}</strong><small>Expression {{ index + 1 }}</small></span></button>
      ...
      <aside class="inspector" aria-label="Character controls">
        ...
        <div ref="inspectorScroll" class="inspector-scroll">
          ...
          <template v-if="tab === 'expressions' && !editing">
            ...
            <div class="expression-grid"><div v-for="e in project.expressions" :key="e.id" :class="['expression-card', { active: selectedExpression === e.id && mode === 'expression' }]"><button class="expression-select" :aria-pressed="selectedExpression === e.id && mode === 'expression'" @click="selectExpression(e)" @dblclick="editExpression(e)"><div class="expression-preview"><Thumb :character="character" :pose="previewPose(e)" :size="92" /></div><div class="expression-card-label"><strong>{{ e.name }}</strong><span>{{ e.beats.length }} beats</span></div></button><ExpressionMenu :name="e.name" :can-delete="project.expressions.length > 1" @rename="renameExpression(e)" @copy="duplicateExpression(e)" @remove="deleteExpression(e)" /></div></div>
          ...
          <template v-if="tab === 'expressions' && editing">
            ...
            <div class="control-section"><label class="field-label" for="expression-name">Expression name</label><input ref="expressionName" id="expression-name" class="text-field" maxlength="60" v-model="expression.name" /><div class="beat-pills"><button v-for="(item, index) in expression.beats" :key="item.id" :class="{ active: index === selectedBeat }" @click="pickBeat(index)">{{ index + 1 }}<span>{{ item.name }}</span></button></div><label class="field-label" for="beat-name">Beat name</label><input ref="beatName" id="beat-name" class="text-field" maxlength="40" v-model="beat.name" /></div>
            <div v-if="isGradientExpression(expression)" class="control-section"><div class="section-heading"><h2>Gradient loop</h2><BeatSectionMenu name="Gradient loop" :can-paste="!!sectionClipboard['Gradient loop']" @copy="copySection('Gradient loop')" @paste="pasteSection('Gradient loop')" /></div><label class="field-label" for="gradient-action">This beat</label><select id="gradient-action" class="text-field" v-model="beat.gradientAction"><option value="rotate">Rotate</option><option value="hold">Hold · no movement</option></select><label v-if="beat.gradientAction === 'rotate'" class="range-control"><span>Rotation speed<output>{{ gradientTurns(beat) }} {{ gradientTurns(beat) === 1 ? 'turn' : 'turns' }} / beat</output></span><input aria-label="Gradient rotations per beat" type="range" min="1" max="8" step="1" :value="gradientTurns(beat)" @input="beat.gradientTurns = Number(($event.target as HTMLInputElement).value)" /></label><p class="panel-hint">{{ beat.gradientAction === 'rotate' ? `${(gradientTurns(beat) / Math.max(.2, beat.duration) * character.speed).toFixed(2)} turns/sec on average. ` : '' }}Adjust beat duration in the timeline. Whole turns keep the loop seamless.</p><label class="field-label" for="pose-source">Face motion</label><select id="pose-source" class="text-field" :value="expression.poseExpressionId ?? ''" @change="setPoseSource"><option value="">Use these beats</option><option v-for="source in project.expressions.filter(e => e.id !== expression.id && !e.poseExpressionId && !isGradientExpression(e))" :key="source.id" :value="source.id">{{ source.name }}</option></select><p v-if="poseSource" class="panel-hint">{{ poseSource.name }} plays throughout the turn and hold.</p><button v-if="poseSource" class="text-button" @click="editExpression(poseSource)"><Icon name="tune" :size="15" />Edit {{ poseSource.name }} motion</button></div><template v-if="!poseSource">
            <div class="control-section"><div class="section-heading"><h2>Pose & props</h2><BeatSectionMenu name="Pose & props" :can-paste="!!sectionClipboard['Pose & props']" @copy="copySection('Pose & props')" @paste="pasteSection('Pose & props')" /></div><label v-for="control in bodyControls" :key="control.key" class="range-control"><span>{{ control.label }}<output>{{ Number(beat.pose[control.key]).toFixed(control.step === 1 ? 0 : 2) }}</output></span><input :aria-label="control.label" type="range" :min="control.min" :max="control.max" :step="control.step" :value="beat.pose[control.key]" @input="updatePose(control.key, $event)" /></label><span class="field-label" id="prop-label">Expression detail</span><div class="prop-tiles" role="group" aria-labelledby="prop-label"><button v-for="prop in PROPS" :key="prop" class="prop-tile" :class="{ active: beat.pose.prop === prop }" :aria-pressed="beat.pose.prop === prop" @click="beat.pose.prop = prop; playing = false; frozenBeat = true"><span class="prop-tile-symbol" aria-hidden="true"><svg v-if="prop === 'sweat'" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M12 3C10 7 5 12 5 16a7 7 0 0 0 14 0c0-4-5-9-7-13Z" /></svg><template v-else>{{ ({ none: '—', zzz: 'Zzz', sparkle: '✧', heart: '♡', question: '?', crown: '♛' })[prop] }}</template></span><span>{{ prop === 'zzz' ? 'Sleep marks' : prop.charAt(0).toUpperCase() + prop.slice(1) }}</span><Icon v-if="beat.pose.prop === prop" name="check" :size="13" /></button></div><div v-if="beat.pose.prop !== 'none'" class="particle-controls"><label class="range-control"><span>Particle size<output>{{ beat.pose.propSize.toFixed(2) }}×</output></span><input aria-label="Particle size" type="range" min="0.5" max="2" step="0.05" :value="beat.pose.propSize" @input="updatePose('propSize', $event)" /></label><label class="range-control"><span>Particle count<output>{{ particleCount(beat.pose.prop, beat.pose.propCount) }}</output></span><input aria-label="Particle count" type="range" min="1" max="6" step="1" :value="particleCount(beat.pose.prop, beat.pose.propCount)" @input="updatePose('propCount', $event)" /></label><label class="range-control"><span>Outward movement<output>{{ beat.pose.propOutward.toFixed(2) }}×</output></span><input aria-label="Outward movement" type="range" min="0" max="3" step="0.05" :value="beat.pose.propOutward" @input="updatePose('propOutward', $event)" /></label><p class="panel-hint">Play the beat to see particles drift away from the body.</p></div><button class="text-button reset-beat" @click="beat.pose = clone(BASE_POSE)"><Icon name="reset" :size="14" />Reset this beat</button></div>
            <div class="control-section"><div class="section-heading"><h2>Eyes</h2><BeatSectionMenu name="Eyes" :can-paste="!!sectionClipboard['Eyes']" @copy="copySection('Eyes')" @paste="pasteSection('Eyes')" /></div><div class="option-chips"><button v-for="eye in EYES" :key="eye" :class="{ active: beat.pose.eye === eye }" @click="beat.pose.eye = eye; playing = false; frozenBeat = true">{{ eyeLabels[eye] }}</button></div><label class="toggle-row compact"><span>White eye dots · eyes +20%</span><input type="checkbox" role="switch" :checked="character.iris" @change="toggleIris" /><span class="switch-track"></span></label><label class="toggle-row compact"><span>Cheek cutouts</span><input type="checkbox" role="switch" v-model="beat.pose.cheeks" /><span class="switch-track"></span></label><label class="toggle-row compact"><span>Link eye sizes</span><input type="checkbox" role="switch" v-model="linkedEyes" /><span class="switch-track"></span></label><div class="sliders"><label v-for="control in numberControls.slice(0, 10)" :key="control.key" class="range-control"><span>{{ control.label }}<output>{{ Number(beat.pose[control.key]).toFixed(control.step === 1 ? 0 : 2) }}</output></span><input :aria-label="control.label" type="range" :min="control.min" :max="control.max" :step="control.step" :value="beat.pose[control.key]" @input="updatePose(control.key, $event)" /></label></div></div>
            <div class="control-section"><div class="section-heading"><h2>Eye placement & rotation</h2><BeatSectionMenu name="Eye placement & rotation" :can-paste="!!sectionClipboard['Eye placement & rotation']" @copy="copySection('Eye placement & rotation')" @paste="pasteSection('Eye placement & rotation')" /></div><label class="toggle-row compact"><span>Mirror eye adjustments</span><input type="checkbox" role="switch" v-model="mirroredEyes" /><span class="switch-track"></span></label><p class="panel-hint">Move and turn each eye independently, or mirror adjustments across the face.</p><div class="sliders"><label v-for="control in eyePlacementControls" :key="control.key" class="range-control"><span>{{ control.label }}<output>{{ beat.pose[control.key] }}{{ control.key.endsWith('Rotation') ? '°' : '' }}</output></span><input :aria-label="control.label" type="range" :min="control.min" :max="control.max" :step="control.step" :value="beat.pose[control.key]" @input="updatePose(control.key, $event)" /></label></div></div>
            <div class="control-section"><div class="section-heading"><h2>Mouth</h2><BeatSectionMenu name="Mouth" :can-paste="!!sectionClipboard['Mouth']" @copy="copySection('Mouth')" @paste="pasteSection('Mouth')" /></div><div class="option-chips"><button v-for="mouth in MOUTHS" :key="mouth" :class="{ active: beat.pose.mouth === mouth }" @click="beat.pose.mouth = mouth; playing = false; frozenBeat = true">{{ mouthLabels[mouth] }}</button></div><label v-for="control in numberControls.slice(10)" :key="control.key" class="range-control"><span>{{ control.label }}<output>{{ Number(beat.pose[control.key]).toFixed(2) }}</output></span><input :aria-label="control.label" type="range" :min="control.min" :max="control.max" :step="control.step" :value="beat.pose[control.key]" @input="updatePose(control.key, $event)" /></label><div class="inline-checks"><label><input type="checkbox" v-model="beat.pose.tongue" />Tongue</label><label><input type="checkbox" v-model="beat.pose.teeth" />Teeth</label><label><input type="checkbox" v-model="beat.pose.drool" />Drool</label><label><input type="checkbox" v-model="beat.pose.tears" />Tears</label></div></div>
            <div class="control-section"><div class="section-heading"><h2>Brows & blush</h2><BeatSectionMenu name="Brows & blush" :can-paste="!!sectionClipboard['Brows & blush']" @copy="copySection('Brows & blush')" @paste="pasteSection('Brows & blush')" /></div><div class="option-chips"><button v-for="brow in (['none','raised','worried','angry'] as const)" :key="brow" :class="{ active: beat.pose.brows === brow }" @click="beat.pose.brows = brow; playing = false; frozenBeat = true">{{ brow.charAt(0).toUpperCase() + brow.slice(1) }}</button></div><label v-for="control in ([{ key: 'browStroke', label: 'Brow stroke width', min: .3, max: 2.5 }, { key: 'browLength', label: 'Brow length', min: .5, max: 2 }] as const)" :key="control.key" class="range-control"><span>{{ control.label }}<output>{{ beat.pose[control.key].toFixed(2) }}×</output></span><input :aria-label="control.label" type="range" :min="control.min" :max="control.max" step="0.05" :value="beat.pose[control.key]" @input="updatePose(control.key, $event)" /></label><label class="range-control"><span>Blush<output>{{ Math.round(beat.pose.blush * 100) }}%</output></span><input aria-label="Blush" type="range" min="0" max="1" step="0.05" :value="beat.pose.blush" @input="updatePose('blush', $event)" /></label></div>
          ...
          <template v-if="tab === 'animations'">
            ...
            <div class="animation-list"><button v-for="a in project.animations" :key="a.id" :class="['animation-card', { active: a.id === selectedAnimation && mode === 'animation' }]" @click="selectAnimation(a)"><Thumb :character="character" :pose="previewPose(stepExpression(a.steps[0]!.expressionId))" :size="54" /><span><strong>{{ a.name }}</strong><small>{{ a.steps.length }} {{ a.steps.length === 1 ? 'expression' : 'expressions' }} · {{ (animationDuration(a) / character.speed).toFixed(1) }}s</small></span><Icon name="play" :size="16" /></button><button v-if="!hasLoadingAnimation" class="animation-card" aria-label="Add Loading animation" @click="addLoading"><Thumb :character="{ ...character, gradient: true }" :pose="previewPose(loadingPreset)" :size="54" /><span><strong>Loading</strong><small>Gradient turn · 0.5s hold</small></span><Icon name="plus" :size="16" /></button></div><button class="text-button new-animation" @click="newAnimation"><Icon name="plus" :size="16" />New animation</button>

          </template>

          <div v-if="(tab === 'expressions' && !editing) || tab === 'animations'" class="control-section motion-section"><div class="section-heading"><h2>Motion</h2><Icon name="tune" :size="16" /></div><label class="range-control"><span>Playback speed<output>{{ character.speed.toFixed(2) }}×</output></span><input aria-label="Playback speed" type="range" min="0.25" max="2" step="0.05" v-model.number="character.speed" /></label><label class="range-control"><span>Body movement<output>{{ Math.round(character.motion * 100) }}%</output></span><input aria-label="Body movement" type="range" min="0" max="1" step="0.05" v-model.number="character.motion" /></label><label class="toggle-row compact"><span>Natural blinking</span><input type="checkbox" role="switch" v-model="character.blink" /><span class="switch-track"></span></label><label v-if="character.blink" class="range-control"><span>Blink interval<output>~{{ character.blinkInterval.toFixed(1) }}s</output></span><input aria-label="Blink interval" type="range" min="1" max="10" step="0.2" v-model.number="character.blinkInterval" /></label><label class="toggle-row compact"><span>Eyes follow</span><input type="checkbox" role="switch" v-model="character.followCursor" /><span class="switch-track"></span></label><label class="toggle-row compact"><span>Body follows</span><input type="checkbox" role="switch" :checked="character.followRotation" @change="toggleRotationFollowing" /><span class="switch-track"></span></label></div>
          ...
          <template v-if="tab === 'export'">
            ...
            <template v-if="exportTab === 'image' || exportTab === 'animation'">
              ...
              <p v-if="exportTab === 'animation' && (character.followCursor || character.followRotation)" class="panel-hint">Rendered loops use a centered cursor. App exports keep live cursor following.</p><div class="export-actions"><button class="button primary full" :disabled="busy" @click="exportMedia"><Icon name="download" :size="17" />{{ busy && !svgAction ? `Rendering ${Math.round(progress * 100)}%` : `Download ${exportTab === 'image' ? 'PNG' : mediaFormat.toUpperCase()}` }}</button><div v-if="exportTab === 'image'" class="two-buttons"><button class="button primary" :disabled="busy" @click="exportSvg('download')"><Icon name="download" :size="17" />{{ svgAction === 'download' ? 'Preparing SVG…' : 'Download SVG' }}</button><button class="button secondary" :disabled="busy" aria-label="Copy SVG to clipboard" title="Copy SVG to clipboard" @click="exportSvg('copy')"><Icon name="copy" :size="15" />{{ svgAction === 'copy' ? 'Copying…' : 'Copy SVG' }}</button></div><template v-if="busy && !svgAction"><progress :value="progress" max="1" aria-label="Export progress"></progress><button class="text-button" @click="controller?.abort()">Cancel export</button></template><p class="panel-hint">{{ width }} × {{ height }} px · current character and orientation</p></div>
            </template>
    ...
    <dialog ref="expressionPicker" class="expression-picker" aria-labelledby="expression-picker-title" @click="($event.target === expressionPicker) && expressionPicker?.close()">
      ...
      <div class="picker-grid"><button v-for="e in project.expressions" :key="e.id" class="picker-expression" @click="addStep(e.id)"><Thumb :character="character" :pose="previewPose(e)" :size="78" /><strong>{{ e.name }}</strong><small>{{ expressionDuration(e).toFixed(1) }}s</small><Icon name="plus" :size="16" /></button></div>
    </dialog>
