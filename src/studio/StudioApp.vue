      ...
      <section class="workbench" aria-label="Character preview and sequencing">
        ...
        <div class="preview-frame" :class="{ light: previewBackground === 'light', checker: previewBackground === 'transparent' }">
          <Stage :character="character" :sample="sample" :rotation="rotation" :zoom="zoom" :background="background" :playing="playing" :preview-size="previewSize" @rotate="rotatePreview" @reset="resetRotation" @pause="playing = false" @cursor="previewCursor = $event"><template #controls>            <div class="orbit-actions" role="group" aria-label="Preview behavior">
              <button class="orbit-action" :disabled="previewSize !== null && previewSize <= 40" :class="{ active: character.trueFront || (previewSize !== null && previewSize <= 40) }" aria-label="Front reset" title="Keep the character facing straight ahead, including during animation" @click="resetFront"><Icon name="front" :size="16" /><span>Front reset</span></button>
              <button class="orbit-action" :disabled="previewSize !== null && previewSize <= 40" :class="{ active: character.lockPosition || (previewSize !== null && previewSize <= 40) }" aria-label="Lock position" :aria-pressed="character.lockPosition" title="Stop floating and breathing; keep expressions and details moving" @click="character.lockPosition = !character.lockPosition"><Icon :name="character.lockPosition ? 'lock' : 'unlock'" :size="16" /><span>Position</span></button>
              <button class="orbit-action" :class="{ active: character.followCursor }" title="Eyes follow cursor" aria-label="Eyes follow" :aria-pressed="character.followCursor" @click="character.followCursor = !character.followCursor"><Icon name="eye" :size="16" /><span>Eyes follow</span></button><button class="orbit-action" :disabled="previewSize !== null && previewSize <= 40" :class="{ active: character.followRotation && (previewSize === null || previewSize > 40) }" title="Character turns toward cursor" aria-label="Body follows" :aria-pressed="character.followRotation" @click="toggleRotationFollowing"><Icon name="cursor" :size="16" /><span>Body follows</span></button>
            </div>
</template></Stage>
      ...
      <aside class="inspector" aria-label="Character controls">
        ...
        <div ref="inspectorScroll" class="inspector-scroll">
          <template v-if="tab === 'character'">
            ...
            <div class="control-section detail-guide"><h2>Responsive detail</h2><p><strong>Above 40 px</strong>Full face and expression details</p><p><strong>16–40 px</strong>Eyes and smile · front locked · no shading</p><p><strong>12 px and below</strong>Gradient and shape only · front locked</p></div>
          </template>
