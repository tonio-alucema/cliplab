...
function restoreSnapshot(value: string) { restoring = true; project.value = JSON.parse(value) as Project; stable = value; restoring = false; persist(); selectedBeat.value = 0; time.value = 0 }
function undo() { if (JSON.stringify(project.value) !== stable) flush(); const previous = history.value.pop(); if (previous) { future.value.push(stable); restoreSnapshot(previous) } }
function redo() { const following = future.value.pop(); if (following) { history.value.push(stable); restoreSnapshot(following) } }
function resetRotation() { rotation.value = { x: -5, y: -12, z: -7 } }
...
function editExpression(value: Expression) { selectExpression(value); editing.value = true; tab.value = 'expressions'; playing.value = false; frozenBeat.value = true }
...
function newAnimation() { const a: Animation = { id: uid('animation'), name: 'New animation', loop: true, steps: [{ id: uid('step'), expressionId: expression.value.id, duration: expressionDuration(expression.value) }] }; project.value.animations.push(a); selectAnimation(a) }
const addExpressionId = ref(project.value.expressions[0]!.id)
function addStep() { const e = project.value.expressions.find(e => e.id === addExpressionId.value); if (e) animation.value.steps.push({ id: uid('step'), expressionId: e.id, duration: expressionDuration(e) }) }
...
async function importProject(event: Event) {
  ...
  try {
    ...
    selectedCharacter.value = imported.characters[0]!.id; selectedExpression.value = imported.expressions[0]!.id; selectedAnimation.value = imported.animations[0]!.id; addExpressionId.value = imported.expressions[0]!.id
    selectedBeat.value = 0; time.value = 0; editing.value = false; frozenBeat.value = false; notify('Project imported. Undo is available if you want to go back.')
      ...
      <section class="workbench" aria-label="Character preview and sequencing">
        ...
        <section class="timeline" aria-label="Animation sequence">
          <div class="timeline-top">
            <div class="timeline-title"><Icon :name="mode === 'expression' ? 'face' : 'animation'" :size="17" /><strong>{{ currentLabel }}</strong><span>{{ mode === 'expression' ? `${expression.beats.length} beats` : `${animation.steps.length} expressions` }}</span></div>
      ...
      <aside class="inspector" aria-label="Character controls">
        ...
        <div ref="inspectorScroll" class="inspector-scroll">
          ...
          <template v-if="tab === 'animations'">
            ...
            <div class="control-section"><div class="section-heading"><h2>Selected animation</h2></div><label class="field-label" for="animation-name">Name</label><input id="animation-name" class="text-field" v-model="animation.name" maxlength="60" /><label class="toggle-row"><span>Loop continuously</span><input type="checkbox" role="switch" v-model="animation.loop" /><span class="switch-track"></span></label><label class="field-label" for="append-expression">Add an expression</label><div class="inline-fields"><select id="append-expression" class="text-field" v-model="addExpressionId"><option v-for="e in project.expressions" :key="e.id" :value="e.id">{{ e.name }}</option></select><button class="button secondary" @click="mode = 'animation'; addStep()"><Icon name="plus" :size="16" />Add</button></div><p class="panel-hint">Drag sequence blocks to reorder. Each block has its own duration.</p></div>
          </template>

          <div v-if="(tab === 'expressions' && !editing) || tab === 'animations'" class="control-section motion-section"><div class="section-heading"><h2>Motion</h2><Icon name="tune" :size="16" /></div><label class="range-control"><span>Playback speed<output>{{ character.speed.toFixed(2) }}×</output></span><input aria-label="Playback speed" type="range" min="0.25" max="2" step="0.05" v-model.number="character.speed" /></label><label class="range-control"><span>Body movement<output>{{ Math.round(character.motion * 100) }}%</output></span><input aria-label="Body movement" type="range" min="0" max="1" step="0.05" v-model.number="character.motion" /></label><label class="toggle-row compact"><span>Natural blinking</span><input type="checkbox" role="switch" v-model="character.blink" /><span class="switch-track"></span></label><label v-if="character.blink" class="range-control"><span>Blink interval<output>~{{ character.blinkInterval.toFixed(1) }}s</output></span><input aria-label="Blink interval" type="range" min="1" max="10" step="0.2" v-model.number="character.blinkInterval" /></label><label class="toggle-row compact"><span>Follow cursor</span><input type="checkbox" role="switch" v-model="character.followCursor" /><span class="switch-track"></span></label></div>
