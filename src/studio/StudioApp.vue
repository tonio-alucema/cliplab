...
import { BASE_POSE, EYES, MOUTHS, PALETTES, PROPS, SHAPES, addLoadingAnimation, animationDuration, clone, defaultExpressions, defaultProject, definitionOf, expressionDuration, isGradientExpression, parseProject, sampleDefinition, uid, type Animation, type Character, type Expression, type Pose, type Project } from './model'
      ...
      <aside class="inspector" aria-label="Character controls">
        ...
        <div ref="inspectorScroll" class="inspector-scroll">
          ...
          <template v-if="tab === 'expressions' && editing">
            ...
            <div v-if="isGradientExpression(expression)" class="control-section"><div class="section-heading"><h2>Gradient loop</h2></div><label class="field-label" for="gradient-action">This beat</label><select id="gradient-action" class="text-field" v-model="beat.gradientAction"><option value="rotate">Rotate · full turn</option><option value="hold">Hold · no movement</option></select><p class="panel-hint">Each turn eases strongly in and out. Adjust Rotate and Hold durations in the timeline.</p></div><div class="control-section"><div class="section-heading"><h2>Face styles</h2></div><div class="face-set-tabs"><button v-for="set in FACE_SETS" :key="set.id" :class="{ active: activeFaceSet === set.id }" :aria-pressed="activeFaceSet === set.id" @click="activeFaceSet = set.id">{{ set.name }}</button></div><p class="face-library-description">{{ faceSet.description }}</p><div class="face-presets"><button v-for="preset in faceSet.presets" :key="preset.id" :aria-label="`Apply ${preset.name} face`" @click="applyFace(preset)"><Thumb :character="{ ...character, trueFront: true }" :pose="facePose(faceSet, preset)" :size="60" /><span>{{ preset.name }}</span></button></div></div>
