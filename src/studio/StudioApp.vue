...
const previewSelection = ref<number | '16-A' | null>(null)
const previewSize = computed(() => previewSelection.value === '16-A' ? 16 : previewSelection.value)
const previewSizes = [16, '16-A', 20, 24, 32, 40, 48, 64, 96, 128, 256, 512] as const
...
watch(exportSize, value => { if (value === 'square') { width.value = 1080; height.value = 1080 } else if (value === 'portrait') { width.value = 1080; height.value = 1350 } else if (value === 'story') { width.value = 1080; height.value = 1920 } else if (value === 'landscape') { width.value = 1920; height.value = 1080 } else if (value === '16' || value === '16-a') { width.value = 16; height.value = 16 } else if (value === 'icon') { width.value = 128; height.value = 128 } })
...
async function exportSvg(action: 'download' | 'copy') {
  ...
  try {
    ...
    const svg = renderSvg(clone(definition.value), { width: width.value, height: height.value, sizeVariant: exportSize.value === '16-a' ? '16-A' : undefined, animationId: animation.value.id, background: transparent.value ? null : exportBackground.value, rotation: { ...rotation.value }, zoom: zoom.value, cursor: clone(previewCursor.value), sample: clone(sample.value) })
    if (action === 'copy') {
    ...
    <main class="studio-workspace">
      <section class="workbench" aria-label="Character preview and sequencing">
        ...
        <div class="preview-frame" :class="{ light: previewBackground === 'light', checker: previewBackground === 'transparent' }">
          <Stage :character="character" :sample="sample" :rotation="rotation" :zoom="zoom" :background="background" :playing="playing" :preview-size="previewSize" :size-variant="previewSelection === '16-A' ? '16-A' : undefined" @rotate="rotatePreview" @reset="resetRotation" @pause="playing = false" @cursor="previewCursor = $event"><template #controls>            <div class="orbit-actions" role="group" aria-label="Preview behavior">
          ...
          <div class="preview-backgrounds" role="group" aria-label="Preview background"><span>Background</span><button v-for="option in previewBackgrounds" :key="option" :class="['background-choice', option, { selected: previewBackground === option }]" :aria-label="`${option.charAt(0).toUpperCase() + option.slice(1)} preview background`" :title="`${option.charAt(0).toUpperCase() + option.slice(1)} background`" :aria-pressed="previewBackground === option" @click="previewBackground = option"><span aria-hidden="true"></span></button></div>
            <div class="stage-tool-actions"><button class="stage-icon" aria-label="Zoom out" :disabled="previewSize !== null || zoom <= .65" @click="zoom = Math.max(.65, zoom - .1)"><Icon name="minus" :size="16" /></button><span class="zoom-label">{{ previewSize ? `${previewSelection} px` : `${Math.round(zoom * 100)}%` }}</span><button class="stage-icon" aria-label="Zoom in" :disabled="previewSize !== null || zoom >= 1.4" @click="zoom = Math.min(1.4, zoom + .1)"><Icon name="plus" :size="16" /></button><button class="stage-icon photo-button" aria-label="Export this pose as an image" title="Photo mode" @click="exportTab = 'image'; tab = 'export'; playing = false; if (previewSelection === '16-A') exportSize = '16-a'"><Icon name="camera" /></button></div>
          </div>
        </div>
        </div>
        ...
        <div class="size-strip">
          ...
          <div class="size-samples" aria-label="Preview sizes"><button class="size-preset fit-preset" :class="{ active: previewSelection === null }" :aria-pressed="previewSelection === null" @click="previewSelection = null"><Icon name="expand" :size="15" />Fit</button><button v-for="size in previewSizes" :key="size" class="size-preset" :class="{ active: previewSelection === size }" :aria-label="`Preview at ${size === '16-A' ? '16-A · 16 pixels, face 20% larger' : `${size} pixels`}`" :aria-pressed="previewSelection === size" @click="previewSelection = size">{{ size }}<span>px</span></button></div>
        </div>
      ...
      <aside class="inspector" aria-label="Character controls">
        ...
        <div ref="inspectorScroll" class="inspector-scroll">
          ...
          <template v-if="tab === 'expressions' && editing">
            ...
            <div class="control-section"><div class="section-heading"><h2>Pose & props</h2><BeatSectionMenu name="Pose & props" :can-paste="!!sectionClipboard['Pose & props']" @copy="copySection('Pose & props')" @paste="pasteSection('Pose & props')" /></div><label v-for="control in bodyControls" :key="control.key" class="range-control"><span>{{ control.label }}<output>{{ Number(beat.pose[control.key]).toFixed(control.step === 1 ? 0 : 2) }}</output></span><input :aria-label="control.label" type="range" :min="control.min" :max="control.max" :step="control.step" :value="beat.pose[control.key]" @input="updatePose(control.key, $event)" /></label><label class="field-label" for="prop-select">Expression detail</label><select id="prop-select" class="text-field" v-model="beat.pose.prop"><option v-for="prop in PROPS" :key="prop" :value="prop">{{ prop === 'zzz' ? 'Sleep marks · Zzz' : prop.charAt(0).toUpperCase() + prop.slice(1) }}</option></select><button class="text-button reset-beat" @click="beat.pose = clone(BASE_POSE)"><Icon name="reset" :size="14" />Reset this beat</button></div>
