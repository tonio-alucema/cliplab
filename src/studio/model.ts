...
export type Eye = 'dot' | 'soft' | 'closed' | 'wink' | 'star' | 'heart' | 'squint' | 'wide'
export type Mouth = 'smile' | 'open' | 'line' | 'frown' | 'oh' | 'wave' | 'sleep'
...
export interface Pose {
  ...
  tongue: boolean; teeth: boolean; drool: boolean
}
...
export interface Character {
  ...
  gradientAngle: number; toon: boolean; eyeColor: string; iris: boolean
...
export interface Sample { pose: Pose; blink: number; bob: number; breathe: number; expressionId: string; beatIndex: number; stepIndex: number }

export const EYES: Eye[] = ['dot', 'soft', 'closed', 'wink', 'star', 'heart', 'squint', 'wide']
export const MOUTHS: Mouth[] = ['smile', 'open', 'line', 'frown', 'oh', 'wave', 'sleep']
...
export const BASE_POSE: Pose = {
  ...
  tongue: false, teeth: false, drool: false
}
...
export function defaultExpressions(): Expression[] {
  return [
    ...
    { id: 'happy', name: 'Happy', description: 'A small open smile says it all.', beats: [
      b('happy-1', 'Light up', .9, { mouth: 'open', tongue: true, mouthOpen: .5, rotationZ: -5, squash: 1.025 }), b('happy-2', 'Delight', 1.3, { eye: 'closed', mouth: 'open', tongue: true, mouthOpen: .75, prop: 'sparkle', rotationZ: 5, squash: .97 }), b('happy-3', 'Glow', 1.5, { mouth: 'open', tongue: true, mouthOpen: .45, rotationZ: -2 })
    ] },
    ...
    { id: 'sleepy', name: 'Sleepy', description: 'Slow breathing, drifting sleep marks.', beats: [
      b('sleep-1', 'Doze', 2, { eye: 'closed', mouth: 'sleep', prop: 'zzz', drool: true, rotationZ: -6, rotationX: 4, squash: .97 }), b('sleep-2', 'Breathe in', 2, { eye: 'closed', mouth: 'oh', mouthOpen: .25, mouthWidth: .65, prop: 'zzz', drool: true, rotationZ: -3, squash: 1.025 }), b('sleep-3', 'Breathe out', 2, { eye: 'closed', mouth: 'sleep', prop: 'zzz', drool: true, rotationZ: -7, squash: .98 })
    ] },
    { id: 'playful', name: 'Playful', description: 'A wink, a grin, a little mischief.', beats: [
      b('play-1', 'Peek', 1.1, { mouth: 'open', tongue: true, rotationY: -10, rotationZ: -8 }), b('play-2', 'Wink', 1.2, { eye: 'wink', mouth: 'open', tongue: true, mouthOpen: .7, rotationZ: 9, prop: 'sparkle' }), b('play-3', 'Grin', 1.3, { mouth: 'open', teeth: true, mouthOpen: .45, rotationZ: -3 })
    ] }
  ]
}
export function defaultProject(): Project {
  const base: Character = {
    ...
    gradientAngle: 20, toon: true, eyeColor: '#172023', iris: false, elevated: false, elevation: .065,
...
export function sampleExpression(expression: Expression, time: number): { pose: Pose; beatIndex: number } {
  ...
  for (let i = 0; i < expression.beats.length; i++) {
    ...
    if (t < beatDuration || i === expression.beats.length - 1) {
      ...
      return { pose: mixPose(previous.pose, beat.pose, eased), beatIndex: i }
    }
    t -= beatDuration
  }
  return { pose: p(), beatIndex: 0 }
}
export function sampleDefinition(def: Definition, animationId: string, time: number, expressionId?: string): Sample {
  ...
  if (previousExpression && blend < 1) sample.pose = mixPose(sampleExpression(previousExpression, expressionDuration(previousExpression) - .00001).pose, sample.pose, blend * blend * (3 - 2 * blend))
  ...
  return { ...sample, expressionId: expression.id, stepIndex, blink, bob: Math.sin(phase) * def.character.motion, breathe: Math.cos(phase) * def.character.motion }
}
